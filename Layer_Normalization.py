...
class Layer_Normalization:
    ...
    def forward_train(self, input_vectors):
        ...
        dividing_factor = np.sqrt(variance + EPSILON)
        self.dividing_factor = dividing_factor

        pregamma = diff / dividing_factor.reshape((-1, 1))
        self.pregamma = pregamma

        output_vectors = pregamma * self.GAMMA.reshape((-1, 1)) + self.BETA.reshape((-1, 1))
    ...
    def backward(self, dC_dY):
        ...
        gamma_adam = Adam(dC_dgamma, self.GAMMA_m_prev, self.GAMMA_v_prev)
        self.GAMMA += gamma_adam[0]
        self.GAMMA_m_prev = gamma_adam[1]
        self.GAMMA_v_prev = gamma_adam[2]

        beta_adam = Adam(dC_dbeta, self.BETA_m_prev, self.BETA_v_prev)
